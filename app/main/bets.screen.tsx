...
export const BetScreen = ({ route }) => {
    ...
    useEffect(() => {
        const fetchBets = async () => {
            try {
                const q = query(collection(firestore, 'bets'), where('groupId', '==', groupId));
    ...
    const addBet = async () => {
        ...
        try {
            const betDoc = await addDoc(collection(firestore, 'bets'), {
                groupId,
                description: newBet,
                createdAt: new Date(),
                votes: 0
    ...
    const placeBets = async () => {
        try {
            ...
            bets.forEach(bet => {
                const betRef = doc(firestore, 'bets', bet.id);
                batch.delete(betRef);
            });
            await batch.commit();
